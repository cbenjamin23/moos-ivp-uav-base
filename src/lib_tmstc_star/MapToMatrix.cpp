...
public:
    ...
    MapTransformer(int argc, char **argv)
    {
        ...
        while (received_map == false)
            ros::spinOnce();
        cmw = coverage_map.info.width;
        cmh = coverage_map.info.height;
        cmres = coverage_map.info.resolution;
        ...
        for (int i = 0; i < robot_num; ++i)
        {
            ...
            // This needs explicit type declaration as Odometry not constptr, check the template yourself
        ...
        // Remove isolated islands from the map with one BFS
        eliminateIslands();
        showMapAndRegionInf();

        // Write experiment-related information
    ...
    bool createPaths(ros::NodeHandle *nh)
    {
        ...
        for (int i = 0; i < robot_num; ++i)
        {
            ...
            int robot_index = (int)((robot_pos[i].first - origin_x) / cmres) + ((int)((robot_pos[i].second - origin_y) / cmres) * cmw); // If the map origin is not (0, 0), then you need to subtract the origin's xy values when converting
        ...
        for (int i = 0; i < paths_idx.size(); ++i)
        {
            ...
            for (int j = 0; j < paths_idx[i].size(); ++j)
            {
                ...
                // Determine the yaw for each goal
                double yaw = 0;
        ...
        while (ros::ok())
        {
            ...
            if (coverage_action)
            {
                for (int i = 0; i < robot_num; ++i)
                {
                    ...
                    if (!goal_ptr[i] || reachGoal(i, goal_ptr[i] - 1) || (*ac_ptr[i]).getState() == actionlib::SimpleClientGoalState::SUCCEEDED || (*ac_ptr[i]).getState() == actionlib::SimpleClientGoalState::ABORTED)
                    {
