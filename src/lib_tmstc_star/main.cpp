...
int main()
{
	...
	// Need to simulate according to the map
	...
	while (getline(infile, line))
	{
		...
		for (int j = 0; j < line.length(); ++j)
		{
			...
			p = line[j] - '0';
			// Note if the map needs to be inverted !!! And for random_n also need to invert, but bath/school don't need
			// p ^= 1;
			// if (p)	paint1(i, j);
			Map[i][len++] = p;
			cout << p << " ";
	...
	// 2021.11.27 If Map is too small, need to reconstruct the original region
	// Division div(Map);
	...
	//  In fact, there's no need to sort
