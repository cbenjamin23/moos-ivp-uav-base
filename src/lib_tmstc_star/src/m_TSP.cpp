...
void MTSP::crossover()
{
    ...
    {
        if ((_random() < conf.crossover_pb))
            cross_id.push_back(i);
    }
...
void MTSP::mutation()
{
    for (int i = 0; i < ppl.size(); ++i)
    {
        ...
        {
            if (_random() > 0.5)
                ppl[i] = concatMutate(ppl[i]);
            else
                ppl[i] = reverseMutate(ppl[i]);
...
void MTSP::setRoulette()
{
    for (int i = 0; i < vals.size(); ++i)
        fitness_vals[i] = 1.0 / vals[i];

    double sum = 0;
    for (int i = 0; i < vals.size(); ++i)
        sum += fitness_vals[i];
    for (int i = 0; i < roulette.size(); ++i)
        roulette[i] = fitness_vals[i] / sum;
    for (int i = 1; i < roulette.size(); ++i)
...
vector<int> MTSP::concatMutate(vector<int> individual)
{
    ...
    vector<int> splice;
    for (int i = m; i < n; ++i)
        splice.push_back(individual[i]);
    for (int i = 0; i < m; ++i)
        splice.push_back(individual[i]);
    for (int i = n; i < individual.size(); ++i)
        splice.push_back(individual[i]);
...
double MTSP::evaluate(vector<int> &individual)
{
    ...
    double L2 = 0.0;
    for (int i = 0; i < sum_tbl.size(); ++i)
        L2 += sum_tbl[i] * sum_tbl[i];
...
Mat MTSP::MTSP2Path()
{
    ...
    // Note: added points cannot go through obstacles
