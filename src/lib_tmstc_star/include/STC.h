...
public:
	...
	std::vector<int> getPath()
	{
		...
		for (auto &&p : edgeSetSTC)
		{
			...
			int x1, y1, x2, y2;
			int newc = bigcols;
			get2DCoordinate(ind1, x1, y1);
			get2DCoordinate(ind2, x2, y2);
		...
		// The rc here must be based on the reduced graph, not the whole map
		// Need to check edges between points
