...
public:
	...
	floatMatrix normalizedEuclideanDistance(bool robotR)
	{
		...
		for (int x = 0; x < cols; ++x)
		{
			...
			{
				if (robotR)
					f[y] = BinaryRobot[y][x] == 0 ? 1e30 : 0;
				else
					f[y] = BinaryNonRobot[y][x] == 0 ? 1e30 : 0;
			}
		...
		for (int y = 0; y < rows; ++y)
		{
			...
			{
				if (robotR)
					region[y][x] = (region[y][x] - minV) * (1.0 / (maxV - minV)) + 1;
				else
					region[y][x] = (region[y][x] - minV) * (1.0 / (maxV - minV));
			}
		}
	...
	intMatrix compactLabeling(intMatrix &m, bool zeroAsBg)
	{
		...
		// remap to reorder labels sequentially
		stat[0] = 0;
		int j = 1;
