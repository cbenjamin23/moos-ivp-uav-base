...
## Comparison with Standard MOOS-IvP

### Standard MOOS-IvP Launch (s1_alpha)

**Simple single-script launch:**
```bash
#!/bin/bash
TIME_WARP=1
COMMUNITY="alpha"
...
- All parameters in .moos file
- Direct pAntler launch
- Marine vehicle (boat) simulation

### UAV MOOS-IvP Launch (UAV_Fly)

**Multi-script system with YAML configuration:**
```bash
launch.sh
├── Reads missionConfig.yaml
├── Launches multiple vehicles via launch_vehicle.sh
└── Launches shoreside via launch_shoreside.sh
```
...
- Supports both MOOS simulator and ArduPilot
- UAV-specific behaviors and commands
- Meta-file preprocessing with nsplug
- Field deployment support

### Key Differences Table

| Feature | Standard MOOS-IvP | UAV MOOS-IvP |
|---------|-------------------|--------------|
| Configuration | In .moos file | YAML + .moos |
| Multi-vehicle | Manual setup | Automated from YAML |
| Port assignment | Manual | Automatic calculation |
| Simulator | uSimMarineV22 | uSimMarineV22 OR ArduPilot |
| Autopilot integration | None | pArduBridge (MAVSDK) |
| Launch complexity | Single script | Multi-script system |
| Field deployment | Not designed for | Built-in support |
| Vehicle type | Marine (boat) | Aerial (UAV) |
